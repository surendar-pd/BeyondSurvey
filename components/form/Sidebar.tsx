				...
				{SECTIONS.map((section, idx) => (
					...
					>
						<AccordionItem value='item-1'>
							<AccordionTrigger className='text-sm hover:text-black rounded cursor-pointer p-4 border-l-4 border-l-transparent hover:bg-slate-100'>
								{section.title}
							</AccordionTrigger>
							<AccordionContent>
								{section.sub.map((subSection) => (
									<p
										key={subSection}
										className='transition-all duration-300 pl-8 text-sm text-slate-500 py-4 hover:text-black rounded cursor-pointer hover:border-l-slate-300 border-l-4 border-l-transparent hover:bg-slate-100'
									>
										{subSection}
									</p>
								))}
